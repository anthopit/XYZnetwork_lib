...
def cat_node_feature(data, graph, TN, cat=True, feature='degree_one_hot', num_workers=1):
    ...
    :param cat: Concatenate?
    :param feature: Feature to concatenate
    :param num_workers: Number of workers (shortest paths)
...
def augment_data(data, augment_list=["edge_perturbation", "node_dropping"], mask_ratio=0.1, mask_mean=0.5, mask_std=0.5, edge_ratio=0.1):
    """
    Augments data using torch

    :param data: Data to use
    :param augment_list: List of augments
    :param mask_ratio: Mask ratio
    :param mask_mean: Mask mean
    :param mask_std: Mask standard deviation
    :param edge_ratio: Edge ratio
...
def get_random_anchorset_networkx(graph, c=0.5):
    """
    Returns random anchorset id for shortest path analyses

    :param graph: Graph to use
    :param c: Used for anchorset id
